...
echo '];';
echo 'var t2maxchar="'.(($xblock*2)/(0.35*($yblock/2))).'";';
echo 'var t2size=[8';
$total=($yblock/35)-8;
$count=$total/2;
for ($i=1; $i < $count ; $i++) {
...
{
    //text1
    echo '<div  style="position:absolute;z-index:5;top:'.($coords["text1"][1]*$yblock).'px;left:'.($coords["text1"][0]*$xblock).'px;width:'.(3*$xblock).'px;height:'.(1*$yblock).'px;"><h1 class="bexi_title" style="font-size:'.($yblock/3).'px;line-height:'.(0.85*($yblock/3)).'px;">Text1</h1></div>';
}

if(isset($coords["text2"]))
{
    //text2
    echo '<div  style="position:absolute;z-index:4;top:'.($coords["text2"][1]*$yblock).'px;left:'.($coords["text2"][0]*$xblock).'px;width:'.(2*$xblock).'px;height:'.(1*$yblock).'px;"><h3 class="bexi_subtitle" style="font-size:'.($yblock/4).'px;line-height:'.(0.85*($yblock/4)).'px;">Text2</h3></div>';
}

if(isset($coords["icon"]))
{
    //icon
    echo '<div  style="position:absolute;z-index:3;top:'.($coords["icon"][1]*$yblock).'px;left:'.($coords["icon"][0]*$xblock).'px;width:'.(1*$xblock).'px;height:'.(1*$yblock).'px;"><i id="'.uniqid().'" class="fab fa-jedi-order bexi_icon" style="font-size:'.($yblock/2).'px;"></i></div>';
